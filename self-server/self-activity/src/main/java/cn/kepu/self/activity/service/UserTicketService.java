...
public enum UserTicketService {
    ...
    /*移动端微信支付*/
 /*移动端微信支付-获取oppenid*/
    public Openid Openid(String code) {
        Openid authorize = null;
        try {
//		HttpClient hc = new HttpClient();
            ...
            authorize = GetOpenid.sendSms(url, params);
        } catch (Exception e) {
//			log.error("getOpenid erro message:" + e.getMessage(), e);
     ...
     */
    private Map<String, String> getWXPrepay2(UserTicket userTicket) throws Exception {
        Float totalFee = userTicket.getTotalFee();
        final String sn = generateSN();
        String openid = userTicket.getUser().getUserName();
     ...
     * 通过getPrepayId 下单支付
     */
    public String getPrepayId(UserTicket userTicket) throws Exception {

        Map<String, String> mp = getWXPrepay2(userTicket);
        ...
        if (mp == null) {
            ...
            String paySign = generateSignature(resultmap, apiKey, WXPayConstants.SignType.MD5);
...
//            e.printStackTrace();
        ...
        if (signValid && app_id.equals(appId) && mch_id.equals(mchId) && sn != null) {
            userTicketDao.paymentSuccess(sn);
        }
...
//            e.printStackTrace();
    ...
    private Map<String, String> getWXPrepay(UserTicket userTicket) throws Exception {
        Map<String, String> resultMap = new HashMap();
        Float totalFee = userTicket.getTotalFee();
        final String sn = generateSN();
        String nonce_str = WXPayUtil.generateUUID();
        String timeStamp = Long.toString(WXPayUtil.getCurrentTimestamp());
        String spbill_create_ip = "192.168.1.240";
    ...
    public Map<String, String> pay(UserTicket userTicket) {
        try {
            Map<String, String> map = getWXPrepay(userTicket);
            return map;
        } catch (final Exception e) {
            e.printStackTrace();
    ...
    public BinaryPair<List<UserTicket>, Long> selectUserTicket(Long id, String name, String mobile, String email, int offset, int pageSize) {
        return userTicketDao.selectUserTicket(id, name, mobile, email, offset, pageSize);
    }

    public void sendEmail(Long id) throws Exception {
        List<UserTicket> list = userTicketDao.sendEmail(id);
        int i = 0;

        if (list.size() > 0) {
            SendMail sendMail = new SendMail(smtpHost, userName, password);
            for (UserTicket userTicket : list) {
                if (i <= 10) {
                    sendMail.send("title", "hehe", new InternetAddress(userTicket.getEmail()));
                } else if (i > 10 && i <= 20) {
                    sendMail = new SendMail(smtpHost, userName, password);
                    sendMail.send("title", "hehe", new InternetAddress(userTicket.getEmail()));
                } else if (i > 20 && i <= 30) {
                    sendMail = new SendMail(smtpHost, userName, password);
                    sendMail.send("title", "hehe", new InternetAddress(userTicket.getEmail()));
    ...
    public void sendMobile(Long id) {
        List<UserTicket> list = userTicketDao.sendMobile(id);

        if (list.size() > 0) {
            for (UserTicket userTicket : list) {
                userService.check3(userTicket.getMobile());
            }
        }
    }
    ...
    public void signIn(UserTicket userTicket) {
        userTicketDao.signIn(userTicket);
    }

    /*验证手机号是否存在*/
    public boolean phoneNumberCheck(UserTicket userTicket) {
        return userTicketDao.phoneNumberCheck(userTicket);
    }
}
