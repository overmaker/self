((window, $, layer, SELF) => {
    SELF.registeredModule("video-comment-view", "module/video/video-comment-view.html");
    ...
    let requestVideoCommentList = (vid, offset, count) => {
        ...
        check.bind("click", () => {
            changeVideoCommentList(id).then().finally(() => {
                ...
                loadVideoCommentList(videoId, 0, 10);
            });
        });
    ...
    }, loadVideoCommentList = (vid, offset, count) => {
        pager.pagination("loading");
        requestVideoCommentList(vid, offset, count).then(result => {
    ...
    SELF.subscribe("load-module", (moduleName, vid) => {
        if (moduleName === "video-comment-view") {
            /* 页面被加载的处理 */
            ...
            pager.pagination({
                onSelectPage: (pageNumber, pageSize) => {
                    if (pageNumber > 0) {
                        ...
                        loadVideoCommentList(videoId, (currentPageNumber - 1) * currentPageSize, currentPageSize);
                    }
                }
            });
