((window, $, layer, upload, SELF) => {
    SELF.registeredModule("video-update", "module/video/video-publish.html");
    ...
    let id = undefined; // update viedo ID
...
//        }
      ...
      return video;
        ...
        return new window.Promise((resolve, reject) => {
            ...
            xhr.open("PUT", `/admin/video/${id}`);
            xhr.setRequestHeader("Content-Type", "application/json");
            xhr.send(JSON.stringify(video));
        });
    }, videoUpdate = (video) => {
        requestUpdate(video).then(() => {
            layer.msg("视频信息保存成功");
            SELF.nav("video");
        }, error => {
    ...
    }, requestActivityById = activityId => {
        return new window.Promise((resolve, reject) => {
            const xhr = new window.XMLHttpRequest();
            xhr.onloadstart = () => SELF.startLoadAnimation();
            xhr.onloadend = () => SELF.stopLoadAnimation();
            ...
            xhr.open("GET", `/admin/video/${activityId}`);
            xhr.responseType = "json";
            xhr.setRequestHeader("Accept", "application/json");
            xhr.send();
        });
    ...
    SELF.subscribe("load-module", (moduleName, videoId) => {
        if (moduleName === "video-update") {
            id = videoId;
            initUI();
            window.Promise.all([requestAllVideoType(), requestAllVideoTheme(), requestAllVideoAlbum(), requestSpeaker(0, 10, ""), requestActivityList(0, 10, undefined), requestActivityById(videoId)]).then(list => {
                typeCombobox.combobox("loadData", list[0]["v1"]); // 填充视频类型下拉选项
                themeCombobox.combobox("loadData", list[1]["v1"]); // 填充视频主题下拉选项
//                albumCombobox.combobox("loadData", list[2]["v1"]); // 填充视频专辑下拉选项
                speakerTable.datagrid("loadData", list[3]["v1"]); // 填充视频演讲者下拉选项
                speakerPager.pagination({
                    total: list[3]["v2"],
                    pageNumber: currentSpeakerPageNumber,
                    pageSize: currentSpeakerPageSize
                });
                activityTable.datagrid("loadData", list[4]["v1"]); // 填充视频所属活动下拉选项
                activityPager.pagination({
                    total: list[4]["v2"],
                    pageNumber: currentActivityPageNumber,
                    pageSize: currentActivityPageSize
                ...
                console.log(list[5]);
                let videoObj = list[5]; // 获取视频各字段
                titleTextbox.textbox("setValue", videoObj["title"]);
                if (videoObj["en-subtitle"] !== null && videoObj["en-subtitle"] !== "") {
                    subtitleEnPath = videoObj["en-subtitle"];
                    subtitleEn.filebox("setText", videoObj["en-subtitle"]);
                }
                if (videoObj["zh-subtitle"] !== null && videoObj["zh-subtitle"] !== "") {
                    subtitleZhPath = videoObj["zh-subtitle"];
                    subtitleZh.filebox("setText", videoObj["zh-subtitle"]);
                }
                if (videoObj["es-subtitle"] !== null && videoObj["es-subtitle"] !== "") {
                    subtitleEsPath = videoObj["es-subtitle"];
                    subtitleEs.filebox("setText", videoObj["es-subtitle"]);
                }
                typeCombobox.combobox("setValue", videoObj["type"]["id"]);
                
                for (let i = 0, len = videoObj["themes"].length; i < len; i++) {
                    let themeId = videoObj["themes"][i]["id"],
                            themeName = videoObj["themes"][i]["name"];
                    themeArray.push(themeId);
                    $("#self-video-theme-container").append(`<button data-id="${themeId}" class="layui-btn layui-btn-primary layui-btn-sm">${themeName}&nbsp;&nbsp;<i class="layui-icon layui-icon-close" style="font-weight:bold;"></i></button>`);
                }
                
//                albumCombobox.combobox("setValue", videoObj["album"]["id"]);
                
                for (let i = 0, len = videoObj["speakers"].length; i < len; i++) {
                    let speakerId = videoObj["speakers"][i]["id"],
                            speakerName = videoObj["speakers"][i]["name"];
                    speakerArray.push(speakerId);
                    $("#self-video-speaker-container").append(`<button data-id="${speakerId}" class="layui-btn layui-btn-primary layui-btn-sm">${speakerName}&nbsp;&nbsp;<i class="layui-icon layui-icon-close" style="font-weight:bold;"></i></button>`);
                }
                
                ageCombobox.combobox("setValue", videoObj["age"]);
                
                introduction.txt.html(videoObj["introduction"]);
                
                console.log(videoObj["activity-id"]);
                if (videoObj["activity-id"] && videoObj["activity-id"] !== null) {
                    activityCombogrid.combogrid("setValue", {id: videoObj["activity-id"], title: videoObj["activity-name"]});
                }
                
                thumbnailPath = videoObj["thumbnail"];
                thumbnail.attr("src", videoObj["thumbnail"]);
                
                videoPath = videoObj["path"];
                $("#self-video-player-div").show();
                player = videojs("self-video-player", {
                    "controls": "true"
                }, () => {
                    player.src(videoObj["path"]);
                });
                
//                if (videoObj["vip"] === true) {
//                    vipSwitchbutton.switchbutton("check");
//                }
                if (videoObj["recommend"] === true) {
                    recommendSwitchbutton.switchbutton("check");
                }
                
                draftBtn.bind("click", () => {
                    let videoObj = preUpload(false);
                    if (typeof videoObj === "object") {
                        videoUpdate(videoObj);
                    }
                });
