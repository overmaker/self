((window, $, layer, layuiElement, SELF) => {
    ...
    let requestActivityList = (title, offset, count) => {
        return new window.Promise((resolve, reject) => {
            ...
            xhr.open("PUT", `/admin/activity/list?is-over=true&offset=${offset}&count=${count}`);
        ...
        requestActivityList(title, offset, count).then(result => {
            ...
            $("#activity-badge-over").text(result["v2"]);
    ...
    SELF.subscribe("load-module", moduleName => {
        if (moduleName === "activity") {
            table = $("#self-module-activity-over").datagrid({
                columns: [[
                ...
                toolbar: "#self-module-activity-over-toolbar",
                rownumbers: true,
                pagination: true,
                fitColumns: true,
                singleSelect: true,
            ...
            searchBox = $("#self-module-activity-over-toolbar-name").searchbox({
                label: "活动标题",
                width: 400,
                labelWidth: 100,
                height: 38,
