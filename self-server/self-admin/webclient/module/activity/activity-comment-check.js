((window, $, layer, SELF) => {
    SELF.registeredModule("activity-comment-check", "module/activity/activity-comment-check.html");
    ...
    let requestActivityCommentList = (title, offset, count) => {
        ...
        check.bind("click", () => {
            changeActivityCommentList(id).then().finally(() => {
                [currentPageNumber, currentPageSize] = [1, 10];
                loadActivityCommentList(searchTitle, 0, 10);
            });
        });
        ...
        del.bind("click", () => {
            deleteActivityCommentList(id).then().finally(() => {
                [currentPageNumber, currentPageSize] = [1, 10];
                loadActivityCommentList(searchTitle, 0, 10);
            });
        });
    ...
    }, loadActivityCommentList = (title, offset, count) => {
        pager.pagination("loading");
        requestActivityCommentList(title, offset, count).then(result => {
    ...
    SELF.subscribe("load-module", moduleName => {
        if (moduleName === "activity-comment-check") {
            /* 页面被加载的处理 */
            table = $("#self-module-activity-comment-check").datagrid({
                columns: [
                    [
                        ...
                        {field: "activity", title: "活动名称", width: "25%", align: "center", formatter: (value, row, index) => value.title},
                ...
                toolbar: "#self-module-activity-comment-check-toolbar",
                rownumbers: true,
                pagination: true,
                fitColumns: true,
                fit: true
            ...
            pager.pagination({
                onSelectPage: (pageNumber, pageSize) => {
                    if (pageNumber > 0) {
                        ...
                        loadActivityCommentList(searchTitle, (currentPageNumber - 1) * currentPageSize, currentPageSize);
                    }
                }
            });

            searchBox = $("#self-module-activity-comment-check-toolbar-name").searchbox({
                label: "活动标题",
                width: 500,
                height: 38,
                labelWidth: 100,
            ...
            chaBtn = $(`#self-module-activity-comment-check-toolbar-change`).linkbutton({
                iconCls: "icon-ok",
                width: 100,
                height: 38,
                text: "审核通过"
            ...
            chaBtn.bind("click", () => {
                ...
                for (let i = 0; i < checked.length; i++) {
                    changeActivityCommentList(checked[i].id);
                }
                [currentPageNumber, currentPageSize] = [1, 10];
                loadActivityCommentList(searchTitle, 0, 10);
            });

            delBtn = $(`#self-module-activity-comment-check-toolbar-delete`).linkbutton({
                iconCls: "icon-cancel",
                width: 100,
                height: 38,
                text: "删除评论"
