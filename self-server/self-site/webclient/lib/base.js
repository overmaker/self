 ...
 * @since 1.0
 */
(function (window, namespace, rootName, isDebug) {
    "use strict";
    window[rootName] = window[rootName] || (function (window, rootName) {
        /**
         * 定义命名空间函数
         * 
         ...
         */
        var createNS = function (fullNS) {
            var obj = window, // 定义命名空间遍历对象。初始化指向全局对象，依次向后遍历
                item = undefined,
                index = 0,
                arr = fullNS.split('.');
         ...
         */
        addInvokeListener = function (opCode, func) {
            /*
         ...
         */
        invokeAndWait = function (opCode, inParam, outParam, callback) {
            var listeners = invokeMapping[opCode], // 得到「操作码」对应的回调函数列表
                func = undefined,
                index = 0;
            if (listeners) {
                /*
        ...
        ns.namespace = createNS;
        ns.publish = invokeAndWait;
        ns.subscribe = addInvokeListener;
        ns.invokeMapping = isDebug === true ? invokeMapping : undefined;
        return ns;
    })(window, rootName);
