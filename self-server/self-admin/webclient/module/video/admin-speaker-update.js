((window, $, layer, SELF) => {
    SELF.registeredPopup("admin-speaker-update", "module/video/admin-speaker-add.html");
    let [dialog, speakerPhotoFileBox, speakerImageFileBox, addBtn] = [undefined, undefined, undefined, undefined];
    let [speakerPhotoPath, imagePath] = [undefined, undefined];
    let [addNameTextBox, addMobileTextBox, addIntroductionTextBox] = [undefined, undefined, undefined];

    let updateSpeaker = (name, photo, mobile, image, introduction, id) => {
        return new window.Promise((resolve, reject) => {
            ...
            xhr.onload = () => {
                if (xhr.status === 204) {
                    dialog.dialog("close");
                    SELF.publish("load-admin-speaker-upload");
                } else {
                    reject(xhr.status);
                }
            };
            ...
            xhr.open("PUT", `/admin/speaker/admin`);
        ...
        return new window.Promise((resolve, reject) => {
            ...
            xhr.open("GET", `/admin/speaker/admin?offset=0&count=10&id=${id}`);
            xhr.responseType = "json";
        ...
        requestSpeaker(id).then(result => {
            addNameTextBox.textbox("setValue", result.v1[0].name);
            addMobileTextBox.textbox("setValue", result.v1[0].mobile);
            addIntroductionTextBox.txt.html(result.v1[0].introduction);

            if (result.v1[0].photo === "undefined") {
                speakerPhotoPath = "img/photo.jpg";
                document.getElementById("admin-speaker-photo").src = speakerPhotoPath;
                speakerPhotoFileBox.filebox("setText", "");
            } else {
                speakerPhotoPath = result.v1[0].photo;
                document.getElementById("admin-speaker-photo").src = speakerPhotoPath;
                speakerPhotoFileBox.filebox("setText", result.v1[0].photo);
            }
            
            if (result.v1[0].image === "undefined") {
                imagePath = "img/default.png";
                document.getElementById("admin-speaker-image-image").src = imagePath;
                speakerImageFileBox.filebox("setText", "");
            } else {
                imagePath = result.v1[0].image;
                document.getElementById("admin-speaker-image-image").src = imagePath;
                speakerImageFileBox.filebox("setText", result.v1[0].image);
            }
            
        });
    };

    SELF.subscribe("admin-speaker-update", (id) => {
        ...
        addBtn.bind("click", () => {
            ...
            updateSpeaker(name, speakerPhotoPath, mobile, imagePath, introduction, id);
        });
    });
})(this, $, layer, SELF);
