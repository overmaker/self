((window, $, layer, SELF) => {
    const modules = {};
    let currentModuleName = undefined;

    /*
     * 注册模块
     */
    SELF.registeredModule = SELF.registeredModule || ((name, htmlFilePath) => {
        modules[name] = htmlFilePath;
    });

    /*
     * 页面导航
     */
    SELF.nav = SELF.nav || ((moduleName, param) => {
        const load = (moduleName) => {
            $("#self-admin-module").empty();
            let htmlFilePath = modules[moduleName],
                    htmlText = window.localStorage[htmlFilePath];
            $("#self-admin-module").append(htmlText);
            currentModuleName = moduleName;
            SELF.publish("load-module", moduleName, param);
        };
        if (currentModuleName) {
            SELF.publish("unload-module", currentModuleName, undefined, () => {
                load(moduleName);
            });
        } else {
            load(moduleName);
     ...
     * 注册弹出层
     */
    SELF.registeredPopup = SELF.registeredPopup || ((name, htmlFilePath) => {
        popups[name] = htmlFilePath;
    });

    SELF.getPopupHtml = SELF.getPopupHtml || ((name) => {
        let htmlFilePath = popups[name],
                htmlText = window.localStorage[htmlFilePath];
        return htmlText;
    });
    ...
    SELF.getAreaList = SELF.getAreaList || ((provinceName, cityName) => {
        let provinceIndex = 0,
                cityIndex = 0,
                province = undefined,
                city = undefined;
        for (; provinceIndex < provinceCount; provinceIndex++) {
            province = SELF.provinceList[provinceIndex];
            if (province["name"] === provinceName) {
     ...
     * 日期格式化
     */
    SELF.datatimeFormatter = SELF.datatimeFormatter || ((time) => {
        const date = new Date(time);
        const [y, m, d, h, M, s] = [date.getFullYear(),
            date.getMonth() + 1,
            date.getDate(),
            date.getHours(),
            date.getMinutes(),
    ...
    $.extend($.fn.propertygrid.defaults.editors, {
        moduleEditor: {
            init: (container, options) => {
                return $("<span>").appendTo(container).switchbutton({
                    width: 80,
                    height: "100%",
                    onText: "已授权",
                    offText: "未授权"
            ...
            getValue: target => {
                var v = target.switchbutton("options").checked;
                return v ? 1 : 0;
            },
            ...
            resize: (target, width) => $(target).switchbutton("resize", width)
        }
    });
     ...
     */
    let index;
    SELF.startLoadAnimation = SELF.startLoadAnimation || (() => {
        if (index) {
            layer.close(index);
        }
        index = layer.load(1);
    });
    SELF.stopLoadAnimation = SELF.stopLoadAnimation || (() => {
        if (index) {
            layer.close(index);
        }
    });
     ...
     * 文件上传
     */
    SELF.fileUpload = SELF.fileUpload || ((subPath, file) => {
        return new window.Promise((resolve, reject) => {
            ...
            xhr.ontimeout = event => layer.msg("上传文件超时");
            xhr.upload.onprogress = event => {
                if (event.lengthComputable) {
                    let value = Math.floor((event.loaded / event.total) * 100);
                    $.messager.progress("bar").progressbar("setValue", value);
                }
            };
            xhr.open("POST", `/admin/files/${subPath}`);
    ...
    SELF.getUser = SELF.getUser || (() => {
        return new window.Promise((resolve, reject) => {
            ...
            xhr.open("GET", "/admin/user/");
            xhr.send();
        });
    });

    SELF.isIE = SELF.isIE || (() => window.ActiveXObject || "ActiveXObject" in window);
