((window, $, layer, SELF) => {
    SELF.registeredPopup("task-add", "module/task/task-add.html");
    ...
    let [typeCombobox, statusCombobox, submitBtn, thumbnailPath] = [undefined, undefined, undefined, undefined];

    let  insertTask = (type, title, thumbnail, descript, score, startTime, endTime, status) => {
        return new window.Promise((resolve, reject) => {
            ...
            xhr.open("POST", `/admin/task`);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.send(`type=${type}&title=${title}&thumbnail=${thumbnail}&descript=${descript}&score=${score}&startTime=${startTime}&endTime=${endTime}&status=1`);
        });
    }, requestTaskType = () => {
        return new window.Promise((resolve, reject) => {
            const xhr = new window.XMLHttpRequest();
    ...
    SELF.subscribe("task-add", () => {
        ...
        const thumbnailFilebox = $("#self-moudle-task-add-thumbnail").filebox({
            width: 313,
            height: 25,
            editable: false,
            buttonText: "&nbsp;选择任务海报&nbsp;"
        ...
        submitBtn.bind("click", () => {
            ...
            insertTask(type, title, thumbnailPath, descript, score, Date.parse(startTime), Date.parse(endTime), 1);
        });
