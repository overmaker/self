((window, $, layer, upload, SELF) => {
    SELF.registeredModule("activity-update", "module/activity/activity-upload.html");
    ...
    const initUI = () => {
        ...
        ticketAddbtn.bind("click", () => {
            SELF.publish("activity-ticket-online-add", id);
        });
        ...
        let ticketsData = ticketTable.datagrid("getData");
        ...
        return new window.Promise((resolve, reject) => {
            ...
            xhr.open("PUT", `/admin/activity`);
            xhr.responseType = "text";
            xhr.setRequestHeader("Content-Type", "application/json");
            xhr.send(JSON.stringify(obj));
        });
    }, activityUpdate = (activityObj) => {
        requestUpdate(activityObj).then(() => {
            layer.msg("活动保存成功");
            SELF.nav("activity");
        }, error => {
        ...
        requestDeleteTicket(ticketId).then(result => {
            if (result === false) {
                layer.alert("该票种已经有人购买，无法删除", {icon: 2, title: false});
            }
        }, error => {
            SELF.errorHandler(error);
        }).finally(() => {
        ...
        edit.bind("click", () => {
            SELF.publish("activity-ticket-online-update", ticketData);
        });
    ...
    SELF.subscribe("load-module", (moduleName, activityId) => {
        if (moduleName === "activity-update") {
            ...
            window.Promise.all([requestAllActivityType(), requestThemes(), requestCooperation(0, 10, ""), requestActivity(activityId), requestTicketByActivity(activityId)]).then(list => {
                ...
                sponsorComboGrid.combogrid("setValue", activity["sponsor"]);
                telTextbox.textbox("setValue", activity["tel"]);
                startCalendar.datetimebox("setValue", SELF.datatimeFormatter(activity["start-time"]));
                endCalendar.datetimebox("setValue", SELF.datatimeFormatter(activity["end-time"]));
                provinceComboBox.combobox("select", activity["province"]);
                cityComboBox.combobox("select", activity["city"]);
                if (activity["area"] && activity["area"] !== "") {
                    areaComboBox.combobox("select", activity["area"]);
                }
                ...
                maxnumSpinner.numberspinner("setValue", activity["max-num"]);
                ...
                $("#self-activity-publish-btn").bind("click", () => {
                    layer.confirm("是否发布？", {icon: 3, title: null}, function (index) {
                        layer.close(index);
                        let activityObj = preUpdate(true);
                ...
                $("#self-activity-draft-btn").bind("click", () => {
                    let activityObj = preUpdate(false);
                    if (typeof activityObj === "object") {
                        activityUpdate(activityObj);
                    }
                });
                
                $("#self-activity-preview-btn").bind("click", () => {
                    SELF.nav("activity-detail", id);
                });
