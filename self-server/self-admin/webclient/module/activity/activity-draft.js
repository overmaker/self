((window, $, layer, layuiElement, SELF) => {
    ...
    let requestActivityList = (title, offset, count) => {
        ...
        requestActivityList(title, offset, count).then(result => {
            ...
            $("#activity-badge-draft").text(result["v2"]);
    ...
    SELF.subscribe("load-module", moduleName => {
        if (moduleName === "activity") {
            table = $("#self-module-activity-draft").datagrid({
                columns: [[
                        ...
                        {field: "xxx", title: "操作", width: "150rem;", align: "left", formatter: (value, row, index) => {
                                ...
                                str += `<div><span style="margin:5px 10px 5px 0px;" data-type="op-edit" data-id=${row["id"]}></span></div>`;
                                return str;
                            }}
                    ]],
                toolbar: "#self-module-activity-draft-toolbar",
                rownumbers: true,
                pagination: true,
                fitColumns: true,
                singleSelect: true,
            ...
            searchBox = $("#self-module-activity-draft-toolbar-name").searchbox({
                label: "活动标题",
                width: 400,
                labelWidth: 100,
                height: 38,
